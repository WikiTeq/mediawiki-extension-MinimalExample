...
class ParserHooksTest extends MediaWikiIntegrationTestCase {
	...
	public static function providePageContentModel() {
		// $pagename, $expectedContentModel
		yield 'Empty page name is wikitext' => [ '', CONTENT_MODEL_WIKITEXT ];
		yield 'Invalid' => [ 'Talk:', '' ];
		yield 'Missing' => [ 'PageIsMissing', '' ];
		yield 'Actual check - wikitext' => [ 'PageIsWikitext', CONTENT_MODEL_WIKITEXT ];
		yield 'Actual check - plain text' => [ 'PageIsPlainText', CONTENT_MODEL_TEXT ];
	}
}
